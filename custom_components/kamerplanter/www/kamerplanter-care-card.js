...
const CARE_STYLES = `
        ...
        /* Collapse the last section's trailing margin so the card's bottom
           padding is not doubled — matches upstream card density (issue #64). */
        ha-card > *:last-child {
          margin-bottom: 0;
        }
        .header {
...
/**
 * Kamerplanter Care Card Editor — shared ha-form editor base
...
class KamerplanterCareCard extends HTMLElement {
  ...
  _buildActionButtons(task) {
    const taskKey = escapeAttr(task.task_key || "");
    if (!taskKey) return "";
  ...
  _render() {
    ...
    const dataUnavailable =
      isUnavailable(overdueState) || isUnavailable(dueTodayState);
