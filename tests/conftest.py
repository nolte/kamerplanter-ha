...
def mock_api():
    ...
    with patch("custom_components.kamerplanter.api.KamerplanterApi") as mock_cls:
        ...
        api.async_get_pending_tasks = AsyncMock(
            return_value=load_fixture("tasks_pending.json")
        )
        api.async_get_overdue_tasks = AsyncMock(
            return_value=load_fixture("tasks_overdue.json")
        )
        api.async_get_tanks = AsyncMock(return_value=load_fixture("tanks.json"))
        api.async_get_plant_nutrient_plan = AsyncMock(
            return_value=load_fixture("nutrient_plan.json")
        )
        api.async_get_plant_current_dosages = AsyncMock(
            return_value=load_fixture("current_dosages.json")
        )
